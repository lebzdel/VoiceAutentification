...
using namespace std;
	...
	void Processor::init() {
		try {
			...
			throw "Ссылка на объект не указывает на экземпляр объекта.";
		}
	}
	...
	void Processor::saveWordAsAudio(const std::string& file, const Word& word) const {
		...
		WAV_HEADER headerNew;
		strncpy(headerNew.RIFF, getWavData()->getHeader().RIFF, 4);
		headerNew.ChunkSize = waveSize + sizeof(WAV_HEADER);
		...
		fs.write((char*)&headerNew, sizeof(WAV_HEADER));
